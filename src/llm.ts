...
export const generateCommitMessage = async (
  ...
  const API_KEY: string =
    config.apiKey || process.env.GEMINI_COMMIT_MESSAGE_API_KEY!;
  ...
  try {
    ...
    if (axios.isAxiosError<GeminiErrorResponse>(error)) {
      if (error.response) {
        ...
        console.error(
          chalk.red(
            "Network error: Could not connect to Google API. Check your internet.",
          ),
        );
      } else {
        console.error(
          chalk.red(
            `An unknown error occurred during the LLM request: ${error.message}`,
          ),
        );
      }
    } else {
